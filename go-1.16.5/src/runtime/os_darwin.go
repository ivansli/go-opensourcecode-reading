...
func newosproc(mp *m) {
	...
	//
	// 创建线程。它从mstart_stub开始执行，它执行一些低级的设置，然后调用mstart。
	...
	// pthread_create -> 创建线程
	// Unix环境编程中 对创建线程方法的封装
	// 新创建的线程 从 mstart_stub 开始执行,mp作为参数传递给 mstart_stub
	// mstart_stub -> runtime.mstart
...
func minit() {
	...
	minitSignalMask()
	getg().m.procid = uint64(pthread_self()) // 获取底层线程id
}
