...
func main() {
	...
	// 进入系统调用，退出进程
	// 可以看出 main goroutine 并未返回，而是直接进入系统调用退出进程了
	exit(0)

	// 保护性代码
	// 如果 exit 意外返回，下面的代码会让该进程 crash 死掉
	for {
		var x *int32
		*x = 0
	}
...
func mReserveID() int64 {
	...
	// mnext是下一个可用的id， int64类型，用例分配id给M
	// sched.mnext+1 < sched.mnext 说明不停的+1之后，超过了该类型的最大值，发生了溢出
...
// M结构体进行一些初始化操作
// id、fastrand、gsignal、alllink 等字段
//
...
func wakep() {
	...
	// 有空闲的P，也有空闲的M，说明此时没有P闲着，就直接返回
	//
	// be conservative about spinning threads
	//
	// !atomic.Cas(&sched.nmspinning, 0, 1) 的含义是：
	//	  此时没有空闲M，所以准备创建一个新的M，就先使用原子操作对自选的m数量+1
	//	  加1失败，则说明已经在其他地方被创建了一个新的M或者有M空闲下来了，就不需要再创建了
...
//
// 用户代码中使用go关键字生成一个新的goroutine，调用的就是该方法
//
//go:nosplit
func newproc(siz int32, fn *funcval) {
	...
	systemstack(func() {
		...
		// 当前P
		_p_ := getg().m.p.ptr()
		// 把新生成的goroutine放到执行队列中（可能是P的 优先队列 > 本地队列 > 全局队列）
		runqput(_p_, newg, true)

		// ！！！
		// runtime.main开始之后，mainStarted会被设置为true，即：go启动之后，该值一直为true
		// 尝试唤醒一个 空闲的P 去执行G
		//
		// 每次创建一个新的G都会去尝试唤醒一个 空闲的P 去执行G
		if mainStarted {
			wakep()
		}
	})
...
func newproc1(fn *funcval, argp unsafe.Pointer, narg int32, callergp *g, callerpc uintptr) *g {
	...
	if _p_.goidcache == _p_.goidcacheend {
		...
		// 号段新的开始id以及终止id
		_p_.goidcache -= _GoidCacheBatch - 1 // 开始id，也可理解为下一个可用的id
		_p_.goidcacheend = _p_.goidcache + _GoidCacheBatch // 终止id
	}
	// 给G分配id
...
func retake(now int64) uint32 {
	...
	for i := 0; i < len(allp); i++ {
		...
		// P的状态 _Prunning _Psyscall
		if s == _Prunning || s == _Psyscall {
			// Preempt G if it's running for too long.
			//
			// 每次P上调度执行新的G _p_.schedtick 字段都会加1
			t := int64(_p_.schedtick)

			// 注意：
			// P中sysmon的schedtick值 与 P中schedtick的值进行比较
			// 1.不相等，则说明上次执行sysmon时P上执行的G与当前执行的G不是同一个
			// 2.相等，则说明上次执行sysmon时P上执行的G与当前执行的G是同一个，要判断是否需要抢占
			if int64(pd.schedtick) != t {
				// 把P上schedtick字段值设置到 _p_.sysmontick.schedtick 上去
				// 以便下次执行sysmon时根据该字段 _p_.sysmontick.schedtick 判断是否需要抢占
				pd.schedtick = uint32(t)
				pd.schedwhen = now
			} else if pd.schedwhen+forcePreemptNS <= now {
				// 上次执行sysmon时P上执行的G与当前执行的G是同一个，并且超过了10ms要抢占
				// 抢占P，最终是想P上的M发送信息，可能最终M与P解绑
				preemptone(_p_)
...
// 试图从M的空闲队列获取一个M， 必须对sched.lock加锁
