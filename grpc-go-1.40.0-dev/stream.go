...
// 在该方法中主要是进行初始化
// 其中可以重点关注一下 Compressor 和 Encode 初始化
...
func newClientStreamWithParams(ctx context.Context, desc *StreamDesc, cc *ClientConn, method string, mc serviceconfig.MethodConfig, onCommit, doneFunc func(), opts ...CallOption) (_ iresolver.ClientStream, err error) {
	// 默认的 callinfo，关于rpc的配置信息
	c := defaultCallInfo()

	// 设置了等待就绪，就把 快速失败 failFast 设为设置值取反
	// 默认是 true，即：默认快速失败
	if mc.WaitForReady != nil {
		c.failFast = !*mc.WaitForReady
	}

	// Possible context leak: （可能会 context泄露）
	...
	// 如何当前方法设置了超时时间，则设置ctx，否则设置可以用于cancel的ctx
	...
	// if set.
	//
	// Compressor会优先从 CallOption 中去读取 Compressor 的名称
	// 然后从全局中去查找（前提已注册），
	// 其次从 DialOption中去读取，开发者可以通过 grpc.WithCompressor() 执行
	//
	...
	// 通过 context.WithValue 封装 rpcinfo 到 上下文ctx 中
	...
	// 从负载均衡器 中 获取一个准备就绪的连接，存放在 cs 对象中
	...
	//  a.newStream() 创建 stream 结构体，也包含了头信息的创建等重要步骤
	/////////////////////////////////////////////////////////////
	...
	//
	//  cs.withRetry
	//  1. 获取一个stream结构体，成功之直接返回，不会进入2。失败的话，则执行2，再执行1
	//  2. 再获取一个conn连接
	//
	//  成功之后，在 cs 中就包含了 主要的几个结构
	//  grpc.ClientConn / stream结构体 / 建立好的conn
	//  之后就可以使用cs来发送接收数据了
	///////////////////////////////////////////////////////

	// cs.withRetry， 创建stream，失败带有重试功能
...
//
// 使用传输创建一个新的尝试
// 如果它成功了，那么它将用这个新的尝试替换clientStream的尝试
...
func (a *csAttempt) newStream() error {
	...
	cs.attempt.s = s // csAttempt 持有stream结构体
...
//
// 如果RPC应该重试返回nil; 否则，它将返回操作应该返回的错误。
...
func (cs *clientStream) retryLocked(lastErr error) error {
	...
	for {
		...
		// 走到这里说明 获取连接已经成功
		// 则再次执行 执行 cs.buffer 中的每一个方法
		// 全部成功直接返回
		//
		// TODO 研究 cs.buffer 用处
		// cs.buffer 通过 bufferForRetryLocked 方法添加闭包方法
...
// TODO （追源码）
//  该方法被多处复用
//  1. 创建stream，失败带有重试功能。 op参数：闭包函数，获取一个stream结构体
//  2. 发送消息。 op参数：闭包函数，发送消息
func (cs *clientStream) withRetry(op func(a *csAttempt) error, onSuccess func()) error {
	...
	for {
		...
		// cs.attempt 保存有 客户端 stream流
		// 先保存一份老的，因为重试 执行下面  op(a) 还会再次获取一份新的 attempt
		// 所以下面会比较两次获取的是否一样
		a := cs.attempt
		cs.mu.Unlock()
		...
		// 成功之后执行 onSuccess() 方法
		// 创建stream成功 直接返回
		//（跳过retryLocked，是因为在 进入当前方法 之前已经获取了一个连接）
		...
		// TODO （追源码）
		//  !!! 重要
		//  这里再次获取一个连接，可能是因为担心 连接已经发生变化，再次获取最新的
		/////////////////////////////
...
func (cs *clientStream) SendMsg(m interface{}) (err error) {
	...
	// 客户端不可以进行流发送。则发送一次之后，就不能再发送了
	// 此时 把 cs.sentLast 标识设置为true 标识已经发过最后一个了
	...
	// hdr、payload，hdr中有5个byte的内容，其中第一个byte存储的是是否compress的标志位，
	// 1：压缩， 0：未压缩。剩余4个byte存储的是payload的长度。
...
func (cs *clientStream) RecvMsg(m interface{}) error {
	...
	// 在 RecvMsg 方法中其首先通过 transport.Stream.RecvCompress 读取 hdr
	// 判断是否需要 UnCompress 和 Decoding，然后通过 recv 读取响应的内容
	err := cs.withRetry(func(a *csAttempt) error {
