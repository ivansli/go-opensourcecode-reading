...
func (s *Server) RegisterService(sd *ServiceDesc, ss interface{}) {
	if ss != nil {
		// 在 由proto文件生成的go文件中 sd.HandlerTyp 接口类型指针
		// reflect.TypeOf(sd.HandlerType).Elem() 可以获取对外提供服务的接口类型
		ht := reflect.TypeOf(sd.HandlerType).Elem()

		// 实现对外提供服务接口的 结构体类型
		st := reflect.TypeOf(ss)

		// 自定义的struct ss未实现 接口 sd
		// 则返回错误，因为未实现服务接口
		if !st.Implements(ht) {
...
func (s *Server) handleStream(t transport.ServerTransport, stream *transport.Stream, trInfo *traceInfo) {
	...
	// 这里便使用到了 RegisterService 步骤中的部分信息，校验rpc是否存在
	// 如果存在则继续处理，这里有一元rpc数据处理和流式数据处理
	// 我们进入一元rpc的数据处理 processUnaryRPC
	//
	// 注册逻辑在本文件的 func (s *Server) register(sd *ServiceDesc, ss interface{}) 中
